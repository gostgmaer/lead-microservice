...
export const listLeads = catchAsync(async (req, res) => {
  const { page = 1, limit = 20, status, priority, source, brand, sort = 'createdAt', order = 'desc', assignedTo } = req.query;
  const result = await leadService.getLeads(req.tenantId, {
    page: parseInt(page), limit: parseInt(limit), status, priority, source, brand, sort, order, assignedTo,
  });
...
export const contactLead = catchAsync(async (req, res) => {
  ...
  await leadService.updateLeadStatus(lead, 'contacted', `Contacted by ${req.user.email}`, req.user.id, agentName);
...
export const updateStatus = catchAsync(async (req, res) => {
  ...
  const agentName = req.user.name || req.user.email;
  await leadService.updateLeadStatus(lead, newStatus, note, req.user.id, agentName);
...
export const holdLead = catchAsync(async (req, res) => {
  ...
  const agentName = req.user.name || req.user.email;
  await leadService.putOnHold(req.params.id, req.tenantId, { onHoldReason, resumeDate }, req.user.id, agentName);
...
export const reopenLead = catchAsync(async (req, res) => {
  const { note } = req.body;
  const agentName = req.user.name || req.user.email;
  await leadService.reopenLead(req.params.id, req.tenantId, { note }, req.user.id, agentName);
...
export const markWon = catchAsync(async (req, res) => {
  ...
  const agentName = req.user.name || req.user.email;
  await leadService.markWon(req.params.id, req.tenantId, { note, closedRevenue }, req.user.id, agentName);
...
export const markLost = catchAsync(async (req, res) => {
  ...
  const agentName = req.user.name || req.user.email;
  await leadService.markLost(req.params.id, req.tenantId, { lostReason, note }, req.user.id, agentName);
