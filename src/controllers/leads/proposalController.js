...
export const sendProposal = catchAsync(async (req, res) => {
  ...
  const lead = await leadService.sendProposal(req.params.id, req.tenantId, req.body, req.user.id, agentName, req);
...
export const resendProposal = catchAsync(async (req, res) => {
  ...
  const agentName = req.user.name || req.user.email;
  const { version, proposalNumber } = await leadService.resendProposal(req.params.id, req.tenantId, messageOverride, req.user.id, agentName);
...
export const reviseProposal = catchAsync(async (req, res) => {
  const agentName = req.user.name || req.user.email;
  const { version, proposalNumber } = await leadService.reviseProposal(req.params.id, req.tenantId, req.body, req.user.id, agentName, req);
  return sendSuccess(res, { data: { version, proposalNumber }, message: 'Proposal revised and sent' });
});

// PATCH /api/leads/:id/proposal/accept
export const acceptProposal = catchAsync(async (req, res) => {
  const agentName = req.user.name || req.user.email;
  const lead = await leadService.acceptProposal(req.params.id, req.tenantId, { note: req.body.note }, req.user.id, agentName);
...
export const declineProposal = catchAsync(async (req, res) => {
  ...
  const agentName = req.user.name || req.user.email;
  await leadService.declineProposal(req.params.id, req.tenantId, { declinedReason, note }, req.user.id, agentName);
  return sendSuccess(res, { data: { nextStep: `Consider revising — POST /api/leads/${req.params.id}/proposal/revise` }, message: 'Proposal declined' });
});
...
// PATCH /api/leads/proposal/accept/:accessKey — public, client-side acceptance
export const clientAcceptProposal = catchAsync(async (req, res) => {
  const { signatureName } = req.body;
  const { proposalNumber } = await leadService.clientAcceptProposal(req.params.accessKey, {
    ip: req.ip,
    userAgent: req.headers['user-agent'],
    signatureName,
  });
  return sendSuccess(res, { message: `Proposal ${proposalNumber} has been successfully accepted. We will contact you shortly.` });
});
