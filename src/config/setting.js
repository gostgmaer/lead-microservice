...
const config = {
  ...
  db: {
    uri: process.env.MONGODB_URI || 'mongodb://localhost:27017/lead_microservice',
  },
  email: {
  ...
  auth: {
    serviceUrl: process.env.AUTH_SERVICE_URL || 'http://localhost:3500',
  },
  dashboard: {
  ...
  pipeline: {
    stages: (process.env.LEAD_PIPELINE_STAGES || 'New,Contacted,Qualified,Proposal Sent,Negotiation,Won,Lost,Archived').split(','),
  },
  cors: {
