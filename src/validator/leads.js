...
const { STATUS_ENUM, BUDGET_ENUM, TIMELINE_ENUM, PROJECT_TYPE_ENUM, CATEGORY_ENUM, CONTACT_METHOD_ENUM } = require('../models/Lead');
...
const validateMarkWon = [
  body('note').optional().trim().isLength({ max: 2000 }),
  body('closedRevenue').optional().isFloat({ min: 0 }),
  handleValidationErrors,
];

// ─── Add Attachment (from external upload microservice) ─────────────────────

const validateAddAttachment = [
  // Support both array form and single-object form
  ...
  body('attachments.*.fileId')
    ...
    .notEmpty().withMessage('Each attachment must have a fileId'),
  body('attachments.*.url')
    .if(body('attachments').exists())
    .notEmpty().isURL().withMessage('Each attachment must have a valid url'),
  body('attachments.*.filename')
    .if(body('attachments').exists())
    .notEmpty().withMessage('Each attachment must have a filename'),
  body('attachments.*.size').optional().isInt({ min: 0 }),
  // Single-object form (when `attachments` key is not present)
  body('fileId')
    .if(body('attachments').not().exists())
    .notEmpty().withMessage('fileId is required'),
  body('url')
    .if(body('attachments').not().exists())
    .notEmpty().isURL().withMessage('A valid url is required'),
  body('filename')
    .if(body('attachments').not().exists())
...
module.exports = {
  ...
  validateReviseProposal,
  validateDeclineProposal,
