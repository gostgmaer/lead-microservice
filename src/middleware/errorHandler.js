...
export const errorHandler = (err, req, res, next) => {
  ...
  if (err.name === 'CastError') {
    ...
    // Wrap unknown errors (programming errors, bugs, etc.)
    error = new AppError(
    ...
    // If it's already an operational error (even from a different AppError class),
    // use it as is but ensure it has a statusCode
    error = err;
  ...
  logger.logError(error, {
    ...
    userId: req.user?.id || req.admin?._id?.toString()
  });

  const redactInternalUrls = (str) => {
    if (typeof str !== 'string') return str;
    return str.replace(/https?:\/\/[^\s/]+/g, '[REDACTED_URL]');
  };

  // Build response
  const statusCode = error.statusCode || 500;
  const humanMessage = isDevelopment ? redactInternalUrls(error.message) : getHumanReadableMessage(error);
  const response = {
    success: false,
  ...
  if (isDevelopment) {
    response.error.stack = redactInternalUrls(error.stack);
    response.error.originalError = redactInternalUrls(error.originalError?.message);
    response.debug = {
